...
典型适用场景：
- **盒约束** $a \le x\_i \le b$：投影是逐元素截断
- **单纯形约束** $\sum x\_i = 1,\; x\_i \ge 0$
...
"精度"指当前解 $\mathbf{x}^{(k)}$ 与最优解 $\mathbf{x}^{\ast}$ 之间的目标函数值差距 $f(\mathbf{x}^{(k)}) - f(\mathbf{x}^{\ast})$。"收敛速度"决定了**达到某个精度需要多少步迭代**。不同方法的对比如下：
...
盒约束 $\mathcal{C} = \lbrace\mathbf{x} : a\_i \le x\_i \le b\_i\rbrace$，投影是逐元素截断：
...
1. 对矩阵做**特征值分解**：$\mathbf{X} = \sum\_i \lambda\_i \mathbf{v}\_i \mathbf{v}\_i^T$
2. 把**负特征值截为零**：$\mathcal{P}\_{\mathcal{C}}(\mathbf{X}) = \sum\_i \max(\lambda\_i, 0)\, \mathbf{v}\_i \mathbf{v}\_i^T$
...
**证明：** $\hat{\mathbf{x}}$ 是 $\min\_{\mathbf{z} \in \mathcal{C}} \frac{1}{2}\lVert\mathbf{z} - \mathbf{y}\rVert^2$ 的解。由凸优化的一阶最优性条件（方向导数非负）：
...
**命题：** $\mathbf{x}^{\ast}$ 是 $\min\_\mathcal{C} f(\mathbf{x})$ 的最优解 $\Leftrightarrow$ 对任意 $\alpha > 0$：

$$\mathbf{x}^{\ast} = \mathcal{P}_\mathcal{C}\!\left(\mathbf{x}^{\ast} - \alpha \nabla f(\mathbf{x}^{\ast})\right) \tag{FP}$$

**证明**：

$\mathbf{x}^{\ast}$ 是凸问题的最优解 $\Leftrightarrow$ 一阶最优性条件：

$$\langle \nabla f(\mathbf{x}^{\ast}),\; \mathbf{z} - \mathbf{x}^{\ast} \rangle \ge 0, \quad \forall \mathbf{z} \in \mathcal{C} \tag{OPT}$$

由 (P1)，$\mathbf{x}^{\ast} = \mathcal{P}\_\mathcal{C}(\mathbf{x}^{\ast} - \alpha \nabla f(\mathbf{x}^{\ast}))$ 当且仅当

$$\langle (\mathbf{x}^{\ast} - \alpha \nabla f(\mathbf{x}^{\ast})) - \mathbf{x}^{\ast},\; \mathbf{z} - \mathbf{x}^{\ast} \rangle \le 0, \quad \forall \mathbf{z} \in \mathcal{C}$$

$$\Leftrightarrow \quad \langle -\alpha \nabla f(\mathbf{x}^{\ast}),\; \mathbf{z} - \mathbf{x}^{\ast} \rangle \le 0$$

$$\Leftrightarrow \quad \langle \nabla f(\mathbf{x}^{\ast}),\; \mathbf{z} - \mathbf{x}^{\ast} \rangle \ge 0 \qquad (\alpha > 0)$$
...
记 $\mathbf{x}^{\ast}$ 为最优解，步长 $\alpha = 1/L$。

**目标：** 证明 $f(\mathbf{x}^{(K)}) - f(\mathbf{x}^{\ast}) \le \frac{L\lVert\mathbf{x}^{(0)} - \mathbf{x}^{\ast}\rVert^2}{2K}$。
...
取 $\mathbf{z} = \mathbf{x}^{\ast}$ 并整理：

$$\langle \mathbf{g},\; \mathbf{x}^+ - \mathbf{x}^{\ast} \rangle \le \frac{1}{\alpha}\langle \mathbf{x} - \mathbf{x}^+,\; \mathbf{x}^{\ast} - \mathbf{x}^+ \rangle \tag{S2}$$

### 步骤 C：利用凸性

$f$ 是凸函数，所以：

$$f(\mathbf{x}^{\ast}) \ge f(\mathbf{x}) + \langle \mathbf{g},\; \mathbf{x}^{\ast} - \mathbf{x} \rangle$$

$$\Rightarrow \langle \mathbf{g},\; \mathbf{x} - \mathbf{x}^{\ast} \rangle \le f(\mathbf{x}) - f(\mathbf{x}^{\ast}) \tag{S3}$$

### 步骤 D：组合

由 (S1)：

$$f(\mathbf{x}^+) - f(\mathbf{x}^{\ast}) \le f(\mathbf{x}) - f(\mathbf{x}^{\ast}) + \langle \mathbf{g},\; \mathbf{x}^+ - \mathbf{x} \rangle + \frac{L}{2}\|\mathbf{x}^+ - \mathbf{x}\|^2$$

将 $\langle \mathbf{g},\; \mathbf{x}^+ - \mathbf{x} \rangle = \langle \mathbf{g},\; \mathbf{x}^+ - \mathbf{x}^{\ast} \rangle + \langle \mathbf{g},\; \mathbf{x}^{\ast} - \mathbf{x} \rangle$ 代入，利用 (S3)：

$$f(\mathbf{x}^+) - f(\mathbf{x}^{\ast}) \le \langle \mathbf{g},\; \mathbf{x}^+ - \mathbf{x}^{\ast} \rangle + \frac{L}{2}\|\mathbf{x}^+ - \mathbf{x}\|^2$$

由 (S2)，取 $\alpha = 1/L$：

$$\le L\langle \mathbf{x} - \mathbf{x}^+,\; \mathbf{x}^{\ast} - \mathbf{x}^+ \rangle + \frac{L}{2}\|\mathbf{x}^+ - \mathbf{x}\|^2$$
...
$$\boxed{f(\mathbf{x}^{(k+1)}) - f(\mathbf{x}^{\ast}) \le \frac{L}{2}\left(\|\mathbf{x}^{(k)} - \mathbf{x}^{\ast}\|^2 - \|\mathbf{x}^{(k+1)} - \mathbf{x}^{\ast}\|^2\right)} \tag{Key}$$

### 步骤 E：求和得收敛速率

对 $k = 0, 1, \ldots, K-1$ 求和 (Key)：

$$\sum_{k=0}^{K-1} \left(f(\mathbf{x}^{(k+1)}) - f(\mathbf{x}^{\ast})\right) \le \frac{L}{2}\left(\|\mathbf{x}^{(0)} - \mathbf{x}^{\ast}\|^2 - \|\mathbf{x}^{(K)} - \mathbf{x}^{\ast}\|^2\right)$$

右边是伸缩和（telescoping sum），大量项对消。因为 $\lVert\mathbf{x}^{(K)} - \mathbf{x}^{\ast}\rVert^2 \ge 0$：

$$\sum_{k=0}^{K-1} \left(f(\mathbf{x}^{(k+1)}) - f(\mathbf{x}^{\ast})\right) \le \frac{L}{2}\|\mathbf{x}^{(0)} - \mathbf{x}^{\ast}\|^2$$

由 (Key) 知函数值递减，左边最小项是最后一项，因此：

$$K \cdot \left(f(\mathbf{x}^{(K)}) - f(\mathbf{x}^{\ast})\right) \le \frac{L}{2}\|\mathbf{x}^{(0)} - \mathbf{x}^{\ast}\|^2$$

$$\boxed{f(\mathbf{x}^{(K)}) - f(\mathbf{x}^{\ast}) \le \frac{L\|\mathbf{x}^{(0)} - \mathbf{x}^{\ast}\|^2}{2K} = O(1/K)}$$

## 4.5 强凸情况下的线性收敛

若 $f$ 还是 $\mu$-强凸的（$\nabla^2 f \succeq \mu \mathbf{I}$），强凸给出更紧的下界：

$$f(\mathbf{x}^{\ast}) \ge f(\mathbf{x}) + \langle \nabla f(\mathbf{x}),\; \mathbf{x}^{\ast} - \mathbf{x} \rangle + \frac{\mu}{2}\|\mathbf{x}^{\ast} - \mathbf{x}\|^2$$

将这个更强的条件代入上面的推导，最终得到：

$$\boxed{\|\mathbf{x}^{(K)} - \mathbf{x}^{\ast}\|^2 \le \left(1 - \frac{\mu}{L}\right)^K \|\mathbf{x}^{(0)} - \mathbf{x}^{\ast}\|^2}$$
